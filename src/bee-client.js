...
function BeeClient(chunkDataEndpoint, options) {
    ...
    this.chunkDataEndpoint = 'http://localhost:8080/chunks'
    options = options || {};
}

const toHex = byteArray => Array.from(byteArray, (byte) => ('0' + (byte & 0xFF).toString(16)).slice(-2)).join('')

BeeClient.prototype.uploadData = async (data) => {
    const chunks = []
...
BeeClient.prototype.uploadChunkData = async (data, hash) => {
    const options = {
        headers: {
...
BeeClient.prototype.downloadChunkData = async (hash) => {
...
BeeClient.prototype.downloadChunks = async (hash) => {
    const chunks = []
